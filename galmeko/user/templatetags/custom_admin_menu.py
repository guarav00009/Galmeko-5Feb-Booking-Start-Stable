...
class _Menu:
                ...
                if parent in parent_item:
                    ...
                    for idx, child_item in enumerate(self.children):
                        if parent in child_item:
                            self.children[idx][parent]['children'].append(
                                child)
    ...
    def admin_apps(self, context, r):
        request = context['request']
        # Rearrangment Of Apps Customized by gaurav
        print(context['available_apps'])
        data = [context['available_apps'][3], context['available_apps']
                [1], context['available_apps'][4], context['available_apps'][0],context['available_apps'][2]]
        for app in data:
            if app['name'] != 'Hospital' and app['name'] != 'Vendor' and app['name'] != 'User' and app['name'] != 'Booking':
                ...
                r += '<li class="nav-item has-treeview menu-open"><ul class="nav nav-treeview">'
            ...
            for model in app['models']:
                ...
                if app['name'] == 'Hospital' or app['name'] == 'Vendor' or app['name'] == 'User' or app['name'] == 'Booking':
                    icon = '<i class="nav-icon fas fa-edit"></i>'
                    css = 'style=background-color:#007bff;color:white;'
                ...
                if(request.path == url):
                    r += '<li class="nav-item"><a href="%s" class="nav-link active" %s>%s %s</a></li>' % (
                        url, css, icon, model['name'])
                elif str(app['app_url']) in request.path and app['app_url'] !='/admin/setting/':
                    css = 'style=background-color:#007bff;color:white;'
                    r += '<li class="nav-item"><a href="%s" class="nav-link" %s>%s %s</a></li>' % (
                        url, css, icon, model['name'])
                else:
